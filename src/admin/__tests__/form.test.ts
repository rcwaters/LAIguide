...
describe('renderForm', () => {
    ...
    it('renders a text input for key', () => {
        renderForm(container, makeMed());
        const input = container.querySelector<HTMLInputElement>('input[data-path="key"]');
        expect(input?.value).toBe('test_med');
    });
    ...
    it('renders the shared providerNotifications list', () => {
        renderForm(container, makeMed());
        const list = container.querySelector<HTMLDivElement>('[data-path="guidance.shared.providerNotifications"]');
    ...
    it('renders early guidance minDays as a number input', () => {
        renderForm(container, makeMed());
        const input = container.querySelector<HTMLInputElement>('[data-path="guidance.early.minDays"]');
    ...
    it('renders early guidance daysBeforeDue when present', () => {
        renderForm(container, makeMed());
        const input = container.querySelector<HTMLInputElement>('input[data-path="guidance.early.daysBeforeDue"]');
        expect(input?.value).toBe('2');
    });

    it('renders early guidance guidanceNote when present', () => {
        renderForm(container, makeMed());
        const input = container.querySelector<HTMLInputElement>('input[data-path="guidance.early.guidanceNote"]');
    ...
    it('renders idealSteps for the first tier', () => {
        renderForm(container, makeMed());
        const idealList = container.querySelector<HTMLDivElement>('[data-path="guidance.late.variants.0.tiers.0.guidance.idealSteps"]');
    ...
    it('renders an open-ended tier (maxDays null) with empty value', () => {
        renderForm(container, makeMed());
        const maxDaysInput = container.querySelector<HTMLInputElement>('[data-path="guidance.late.variants.0.tiers.1.maxDays"]');
...
describe('collectFormData', () => {
    ...
    it('reads updated number input value', () => {
        const med = makeMed();
        renderForm(container, med);
        const input = container.querySelector<HTMLInputElement>('input[data-path="guidance.early.minDays"]')!;
        input.value = '28';
        const result = collectFormData(container, med) as { guidance: { early: { minDays: number } } };
    ...
    it('stores null for a cleared number input', () => {
        ...
        const input = container.querySelector<HTMLInputElement>('input[data-path="guidance.early.minDays"]')!;
        input.value = '';
        const result = collectFormData(container, med) as { guidance: { early: { minDays: unknown } } };
    ...
    it('collects list editor items into an array', () => {
        ...
        const result = collectFormData(container, med) as { guidance: { shared: { providerNotifications: string[] } } };
    ...
    it('writes nested values at the correct path', () => {
        const med = makeMed();
        renderForm(container, med);
        const input = container.querySelector<HTMLInputElement>('[data-path="guidance.late.variants.0.tiers.0.maxDays"]')!;
        input.value = '45';
