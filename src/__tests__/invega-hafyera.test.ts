...
describe('invega-hafyera — buildLateInfoRows', () => {
    ...
    describe('date-derived boundaries (via buildLateParams)', () => {
        it('day 180 → early guidance; day 181 → administer; day 203 → consult', () => {
            ...
            expect(g(181).idealSteps.some(s => s.includes('Proceed with administering the Hafyera injection'))).toBe(true);
            expect(g(203).idealSteps.some(s => s.includes('Consult provider prior to proceeding'))).toBe(true);
        });
    });
});
