...
describe('getAbilifyMaintenaGuidance', () => {
    ...
    describe('date-derived boundaries (via buildLateParams)', () => {
        ...
        it('1-2 doses: day 27 → not due; day 28 → routine; day 36 → reinitiation; day 120 → consult', () => {
            const p27 = entry.buildLateParams({ 'last-abilify': localDaysAgo(27), 'abilify-prior-dose-group': '1-2' });
            const p28 = entry.buildLateParams({ 'last-abilify': localDaysAgo(28), 'abilify-prior-dose-group': '1-2' });
            const p36 = entry.buildLateParams({ 'last-abilify': localDaysAgo(36), 'abilify-prior-dose-group': '1-2' });
            const p120 = entry.buildLateParams({ 'last-abilify': localDaysAgo(120), 'abilify-prior-dose-group': '1-2' });
            expect((entry.getLateGuidance(p27) as GuidanceResult).idealSteps.some(s => s.includes('not due'))).toBe(true);
            expect((entry.getLateGuidance(p28) as GuidanceResult).idealSteps.some(s => s.includes('Administer usual Abilify Maintena monthly dose'))).toBe(true);
            expect((entry.getLateGuidance(p36) as GuidanceResult).idealSteps.some(s => s.includes('Re-initiate:'))).toBe(true);
            expect((entry.getLateGuidance(p120) as GuidanceResult).idealSteps.some(s => s.includes('Consult provider first'))).toBe(true);
        });

        it('3+ doses: day 42 → routine; day 43 → reinitiation; day 120 → consult', () => {
            const p42 = entry.buildLateParams({ 'last-abilify': localDaysAgo(42), 'abilify-prior-dose-group': '3+' });
            const p43 = entry.buildLateParams({ 'last-abilify': localDaysAgo(43), 'abilify-prior-dose-group': '3+' });
            const p120 = entry.buildLateParams({ 'last-abilify': localDaysAgo(120), 'abilify-prior-dose-group': '3+' });
            expect((entry.getLateGuidance(p42) as GuidanceResult).idealSteps.some(s => s.includes('Administer usual Abilify Maintena monthly dose'))).toBe(true);
            expect((entry.getLateGuidance(p43) as GuidanceResult).idealSteps.some(s => s.includes('Re-initiate:'))).toBe(true);
            expect((entry.getLateGuidance(p120) as GuidanceResult).idealSteps.some(s => s.includes('Consult provider first'))).toBe(true);
        });
    });
});
