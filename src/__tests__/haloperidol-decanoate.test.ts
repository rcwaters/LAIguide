...
function getHaloperidolGuidance(days: number, variant: string): GuidanceResult {
    return MED_REGISTRY['haloperidol_decanoate'].getLateGuidance({
        daysSince: days,
        variant,
    }) as GuidanceResult;
}
...
describe('getHaloperidolDecanoateGuidance', () => {
    describe('1-3 prior doses (2-tier: ≤27 not-due | ≤84 check-in | 85+ reinitiate)', () => {
        it('≤27 days: not yet due', () => {
            expect(
                getHaloperidolGuidance(20, '1-3').idealSteps.some((s) => s.includes('not yet due')),
            ).toBe(true);
        });
        ...
        it('28–84 days: check-in guidance', () => {
            ...
            expect(
                r.idealSteps.some((s) =>
                    s.includes('Administer usual haloperidol decanoate injection'),
                ),
            ).toBe(true);
        ...
        it('exact boundaries: day 27/28 and day 84/85', () => {
            expect(
                getHaloperidolGuidance(27, '1-3').idealSteps.some((s) => s.includes('not yet due')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(28, '1-3').idealSteps.some((s) => s.includes('6–7 days')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(84, '1-3').idealSteps.some((s) => s.includes('6–7 days')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(85, '1-3').idealSteps.some((s) =>
    ...
    describe('4+ prior doses (3-tier: ≤27 not-due | ≤41 routine | 42–84 check-in | 85+ reinitiate)', () => {
        it('≤27 days: not yet due', () => {
            expect(
                getHaloperidolGuidance(20, '4+').idealSteps.some((s) => s.includes('not yet due')),
            ).toBe(true);
        });
        ...
        it('exact boundaries: day 27/28, day 41/42, day 84/85', () => {
            expect(
                getHaloperidolGuidance(27, '4+').idealSteps.some((s) => s.includes('not yet due')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(28, '4+').idealSteps.some((s) => s.includes('4 weeks')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(41, '4+').idealSteps.some((s) => s.includes('4 weeks')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(42, '4+').idealSteps.some((s) => s.includes('6–7 days')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(84, '4+').idealSteps.some((s) => s.includes('6–7 days')),
            ).toBe(true);
            expect(
                getHaloperidolGuidance(85, '4+').idealSteps.some((s) =>
                    s.includes('Reinitiation is required'),
                ),
            ).toBe(true);
        });
    ...
    describe('date-derived boundaries (via buildLateParams)', () => {
        ...
        it('1-3 doses: day 27 → not due; day 28 → check-in; day 85 → reinitiation', () => {
            const p27 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(27),
                'haloperidol-prior-doses': '1-3',
            });
            const p28 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(28),
                'haloperidol-prior-doses': '1-3',
            });
            const p85 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(85),
                'haloperidol-prior-doses': '1-3',
            });
            expect(
        ...
        it('4+ doses: day 27 → not due; day 41 → routine; day 42 → check-in; day 85 → reinitiation', () => {
            ...
            const p41 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(41),
                'haloperidol-prior-doses': '4+',
            });
            const p42 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(42),
                'haloperidol-prior-doses': '4+',
            });
            const p85 = entry.buildLateParams({
                'last-haloperidol': localDaysAgo(85),
            ...
            expect(
                (entry.getLateGuidance(p41) as GuidanceResult).idealSteps.some((s) =>
                    s.includes('4 weeks'),
                ),
            ).toBe(true);
            expect(
